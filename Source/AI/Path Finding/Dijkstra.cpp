...
bool Dijkstra::getPath(DijkstraNode* start, DijkstraNode* goal, std::vector<DijkstraNode*>& path)
{
	...
	std::make_heap(open.begin(), open.end(), CompareDNodes());
	pushOpen(start);
	start->setDistance(0.0f);

	while (!open.empty())
	{
		std::sort(open.begin(), open.end(), CompareDNodes());

		currentNode = open.front();
		popOpen(currentNode);

		if (currentNode->getType() == goal->getType())
		{
...
{
	open.push_back(node);
	std::push_heap(open.begin(), open.end(), CompareDNodes());
}

void Dijkstra::popOpen(DijkstraNode* node)
{
	std::pop_heap(open.begin(), open.end(), CompareDNodes());
	open.pop_back();
	node->setClosed(true);
